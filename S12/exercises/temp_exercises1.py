...
def parse_arguments():
    ...
    "Devuelve: file_path (str): La ruta del archivo proporcionada por el usuario."
    unixOptions = "f:"
    gnuOptions = ["file="]
...
def load_data(file_path):
    return pd.read_csv(file_path, encoding='latin1', delimiter=';', low_memory=False)
...
def preprocess_data(df, threshold=0.5):
    ...
    #Determina el numero de '*' permitidos con base en el umbral especificado (threshold).
    allowed_stars = int(df.shape[1] * threshold)
    
    # Vectoriza el filtro (más rápido que apply)
    mask = (df.eq('*').sum(axis=1) <= allowed_stars)

    # crear una copia del datafrme
    ...
    new_columns_names = ['PERTENENCIA_ESTABLECIMIENTO_SALUD', 'SEXO', 'GRUPO_EDAD', 'ETNIA', 'GLOSA_PAIS_ORIGEN', 'COMUNA_RESIDENCIA', 'GLOSA_COMUNA_RESIDENCIA',
    ...
    old_columns_names = cleaned_df.columns
    
    column_mapping = dict(zip(old_columns_names, new_columns_names))
...
if __name__ == "__main__":
    # Analizar la ruta del archivo a partir de los argumentos de la linea de comandos
    file_path = parse_arguments()
    table_name = 'egresos_pacientes'
    
    # Carga la base de datos en tu sistema 
    engine = create_db_engine('C:/Git/vehicles_env/database/ministerio_de_salud_chile.db')
